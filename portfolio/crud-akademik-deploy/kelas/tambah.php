...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $pdo->prepare("INSERT INTO kelas (nama_kelas,jurusan) VALUES (?,?)")->execute([$nama_kelas, $jurusan]);
        flash('success',"Kelas $nama_kelas berhasil ditambahkan.");
...
<div class="container mt-5 pt-4" style="max-width:480px">
    <h4 class="fw-bold mb-4 mt-2">Tambah Kelas</h4>
    ...
    <div class="card border-0 shadow-sm"><div class="card-body p-4">
        <form method="POST">
            ...
            <input type="text" name="nama_kelas" class="form-control" value="<?=htmlspecialchars($_POST['nama_kelas']??'')?>" required></div>
            <div class="mb-4"><label class="form-label fw-semibold">Jurusan *</label>
            <input type="text" name="jurusan" class="form-control" value="<?=htmlspecialchars($_POST['jurusan']??'')?>" required></div>
