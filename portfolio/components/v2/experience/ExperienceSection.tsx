...
export default function ExperienceSection() {
  ...
  return (
    <section id="experience" className="relative w-full min-h-[90vh] bg-white text-[#222] py-24 md:py-32 overflow-hidden border-t-2 border-gray-100 flex flex-col justify-center">
      ...
      <div className="relative z-10 w-[95%] md:w-[90%] xl:w-[80%] max-w-[1200px] mx-auto flex flex-col h-full justify-center">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 md:mb-24">
          <div>
            <p className="font-mono text-sm tracking-[0.2em] text-[#E74C3C] mb-4 font-bold">CHAPTER II</p>
            <h2 className="text-5xl md:text-7xl lg:text-[6rem] font-black uppercase tracking-tighter text-gray-900 leading-none">
              History.
            </h2>
          </div>
          <p className="text-gray-500 max-w-sm text-sm md:text-base font-medium leading-relaxed pb-2">
            A minimalist archive of my professional footprint. Click on any role to expand its details.
          </p>
        </div>

        {/* Accordion List */}
        <div className="w-full flex flex-col border-t-2 border-gray-900">
          {experiences.map((exp) => {
            const isExpanded = expandedId === exp.id;

            return (
              <div key={exp.id} className="w-full border-b border-gray-200">
                
                {/* Accordion Header (Clickable) */}
                <button
                  onClick={() => toggleAccordion(exp.id)}
                  className="w-full py-8 md:py-12 flex flex-col md:flex-row md:items-center justify-between text-left group transition-colors duration-300 hover:bg-gray-50/50"
                >
                  <div className="flex flex-col gap-2 transform transition-transform duration-500 group-hover:translate-x-4">
                    <h3 className={`text-3xl md:text-5xl lg:text-6xl font-black uppercase tracking-tighter transition-colors duration-300 ${isExpanded ? 'text-[#E74C3C]' : 'text-gray-900'}`}>
                      {exp.company}
                    </h3>
                  ...
                  <div className="mt-6 md:mt-0 flex items-center gap-6 transform transition-transform duration-500 group-hover:-translate-x-4">
                    <span className="font-mono text-sm md:text-base font-bold text-gray-500 tracking-widest bg-gray-100 px-4 py-2 rounded-full">
                      {exp.year}
                    </span>
                    
                    {/* Plus / Minus Icon */}
                    <div className="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center relative transition-colors duration-300 group-hover:border-gray-900">
                      <div className="w-3 h-[2px] bg-gray-900 absolute" />
                      <div className={`w-3 h-[2px] bg-gray-900 absolute transition-transform duration-300 ${isExpanded ? 'rotate-0 opacity-0' : 'rotate-90'}`} />
                    </div>
                  </div>
                </button>
                    ...
                    >
                      <div className="w-full flex flex-col lg:flex-row gap-12 pb-12 pt-4 px-4 md:px-8">
                        ...
                        <div className="w-full lg:w-5/12">
                          <h5 className="text-xs font-mono tracking-widest text-[#E74C3C] font-bold uppercase mb-4">
                            The Role
                          </h5>
                          ...
                          <div className="flex flex-wrap gap-2 mt-8">
                            {exp.tech.map((t, i) => (
                              <span key={i} className="px-4 py-1.5 bg-gray-900 text-white rounded-md text-xs font-bold tracking-wide">
                                {t}
                              </span>
                            ))}
                          </div>
                          ...
                          <ul className="space-y-4">
                            {exp.achievements.map((achievement, i) => (
                              <li key={i} className="flex items-start group/item">
                                <span className="mr-4 text-[#E74C3C] text-xl leading-none mt-0.5 transition-transform group-hover/item:translate-x-1">→</span>
                                <span className="text-gray-600 text-base font-medium leading-relaxed">
                                  {achievement}
                                </span>
                              </li>
                            ))}
