...
export default function SkillsSection() {
  return (
    <section id="skills" className="relative w-full min-h-screen bg-[#F3F0EC] text-[#222] py-20 overflow-hidden border-t-2 border-gray-300 flex flex-col justify-center">
      ...
      <div className="relative z-10 w-[95%] md:w-[90%] xl:w-[85%] max-w-[1400px] mx-auto flex flex-col h-full justify-center">
        ...
        <div className="text-center md:text-left mb-10 md:mb-12">
          <p className="font-mono text-xs md:text-sm tracking-[0.2em] text-[#E74C3C] mb-2 font-bold">THE ARSENAL</p>
          <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight text-gray-900 mb-4">
            Bookshelf.
          </h2>
          <p className="text-gray-600 max-w-2xl text-xs md:text-sm font-medium leading-relaxed">
          ...
          {Object.entries(categorizedIcons).map(([category, icons], shelfIdx) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-6">
                <h3 className="text-xl md:text-2xl font-black text-gray-900 uppercase tracking-tighter whitespace-nowrap">
                  {category}
                </h3>
              ...
              >
                {icons.map((icon, idx) => (
                  <motion.div 
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="group relative w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-white rounded-[22%] sm:rounded-[25%] shadow-[0_3px_8px_rgba(0,0,0,0.05)] border border-gray-100/50 flex flex-col items-center justify-center p-2.5 sm:p-3 md:p-4 transition-shadow hover:shadow-[0_12px_24px_rgba(0,0,0,0.1)] cursor-pointer"
                  >
                    {/* Inner highlight for a subtle "glass/plastic" feel */}
                    <div className="absolute inset-0 rounded-[22%] sm:rounded-[25%] bg-gradient-to-br from-white/80 to-transparent pointer-events-none" />
                    ...
                    <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gray-900 text-white text-[10px] sm:text-xs font-bold px-2 py-1 rounded-md whitespace-nowrap pointer-events-none z-50">
                      {icon.name}
                      {/* Tooltip triangle */}
                      <div className="absolute bottom-[-4px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-t-[4px] border-t-gray-900" />
                    </div>
                  </motion.div>
                ))}
              </motion.div>
