  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
        ...
        />
        {/* Gradient overlays */}
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(6,14,30,0.92) 0%, rgba(6,14,30,0.65) 50%, rgba(6,14,30,0.25) 100%)" }} />
        <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse at 20% 50%, ${ac}12, transparent 60%)` }} />
      </div>
      ...
      <div style={{
        ...
        transform: curtainDone ? "scaleX(0)" : "scaleX(1)",
        transition: curtainDone ? `transform 0.85s cubic-bezier(.77,0,.18,1) ${index * 100}ms` : "none",
        pointerEvents: "none",
      }} />
      ...
      <div
        ...
        style={{ position: "relative", zIndex: 2, padding: "40px 48px", display: "flex", flexDirection: "column", justifyContent: "center", minHeight: 340 }}
      >
        {/* Top accent */}
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(to right,${ac},${ac}50,transparent)` }} />

        {/* Period badge */}
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 16, width: "fit-content" }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: ac, boxShadow: `0 0 8px ${ac}` }} />
          <span style={{ fontSize: 10.5, color: ac, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600, letterSpacing: "0.5px" }}>{org.period}</span>
        </div>
        ...
        <h3 style={{ fontFamily: "'Outfit',sans-serif", fontSize: "clamp(20px,2.5vw,28px)", fontWeight: 800, color: "#FFFFFF", marginBottom: 12, lineHeight: 1.2, letterSpacing: "-0.5px", maxWidth: 520 }}>{org.role}</h3>
        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.55)", lineHeight: 1.8, marginBottom: 20, maxWidth: 500, fontFamily: "'Outfit',sans-serif" }}>{org.desc}</p>
        ...
        <a href={org.instagram} target="_blank" rel="noreferrer" style={{
          ...
          fontSize: 12.5, fontWeight: 600, color: hovered ? "var(--white)" : "rgba(255,255,255,0.6)",
          textDecoration: "none", padding: "8px 16px", borderRadius: 9,
          background: hovered ? `${ac}18` : "rgba(255,255,255,0.05)",
          border: `1px solid ${hovered ? ac + "35" : "rgba(255,255,255,0.1)"}`,
