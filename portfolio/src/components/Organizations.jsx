...
const OrgSection = ({ org, index, visible, seeIg }) => {
...
const Organizations = () => {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([e]) => { setVisible(e.isIntersecting); }, { threshold: 0.04 });
    ...
    <section id="organizations" ref={ref} style={{ background:"var(--navy-2)", borderTop:"1px solid rgba(59,130,246,0.07)" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        {data.organizations.map((org, i) => (
          <OrgSection key={org.role} org={org} index={i} visible={visible} seeIg={t.see_ig} />
        ))}
      </div>
    </section>
  );
