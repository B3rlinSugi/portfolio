...
const PhotoTilt = ({ started }) => {
  const wrapRef = useRef(null), rafRef = useRef(null);
  ...
  return (
    <div ref={wrapRef} onMouseEnter={() => setHovered(true)} onMouseMove={onMove} onMouseLeave={onLeave}
      style={{ perspective: "900px", opacity: started ? 1 : 0, transform: started ? "translateY(0)" : "translateY(24px)", transition: "opacity 0.7s ease 0.2s, transform 0.7s cubic-bezier(.22,1,.36,1) 0.2s", cursor: "default" }}>
      <div style={{ position: "absolute", inset: -24, background: `radial-gradient(ellipse at ${tilt.gx}% ${tilt.gy}%, rgba(29,78,216,0.25), rgba(6,182,212,0.1) 50%, transparent 70%)`, borderRadius: 32, filter: "blur(20px)", transition: hovered ? "background 0.12s" : "background 0.5s", pointerEvents: "none" }} />
      <div style={{ position: "absolute", top: 14, left: 14, right: -14, bottom: -14, borderRadius: 22, background: "linear-gradient(135deg,rgba(29,78,216,0.12),rgba(6,182,212,0.06))", border: "1px solid rgba(59,130,246,0.12)", transform: hovered ? `perspective(900px) rotateX(${tilt.rx * 0.4}deg) rotateY(${tilt.ry * 0.4}deg) translate(${tilt.ry * 0.4}px, ${-tilt.rx * 0.3}px)` : "none", transition: hovered ? "transform 0.1s linear" : "transform 0.55s cubic-bezier(.22,1,.36,1)" }} />
      <div style={{ position: "relative", zIndex: 2, borderRadius: 20, overflow: "hidden", border: `1.5px solid ${hovered ? "rgba(59,130,246,0.5)" : "rgba(59,130,246,0.25)"}`, boxShadow: hovered ? "0 40px 80px rgba(0,0,0,0.6),0 0 0 1px rgba(59,130,246,0.15),inset 0 1px 0 rgba(255,255,255,0.06)" : "0 20px 60px rgba(0,0,0,0.45)", transform: hovered ? `perspective(900px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg) scale(1.03) translateZ(12px)` : "perspective(900px) rotateX(0) rotateY(0) scale(1)", transition: hovered ? "transform 0.1s linear,box-shadow 0.2s,border-color 0.2s" : "transform 0.6s cubic-bezier(.22,1,.36,1),box-shadow 0.4s,border-color 0.3s", willChange: "transform" }}>
        <div style={{ position: "absolute", inset: 0, zIndex: 4, borderRadius: 20, pointerEvents: "none", background: hovered ? `radial-gradient(circle at ${tilt.gx}% ${tilt.gy}%, rgba(255,255,255,0.09), transparent 55%)` : "transparent", transition: hovered ? "background 0.1s" : "background 0.4s" }} />
        <img src="/foto2.jpg" alt="Berlin Sugiyanto" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", objectPosition: "center top", display: "block" }} onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }} />
      ...
      <div style={{ position: "absolute", bottom: -20, left: "50%", transform: "translateX(-50%)", background: "rgba(10,22,40,0.97)", border: "1px solid rgba(59,130,246,0.22)", borderRadius: 12, padding: "10px 18px", display: "flex", alignItems: "center", gap: 10, backdropFilter: "blur(14px)", whiteSpace: "nowrap", boxShadow: "0 8px 32px rgba(0,0,0,0.45)", zIndex: 5, animation: "floatBadge 5s ease-in-out infinite" }}>
        <img src="https://upload.wikimedia.org/wikipedia/id/thumb/7/79/Logo_Universitas_Gunadarma.png/220px-Logo_Universitas_Gunadarma.png" alt="UG" style={{ height: 26, objectFit: "contain", flexShrink: 0 }} onError={e => { e.target.style.display = "none"; }} />
        <div>
...
/* ── Radar / Spider Chart ── */
const RadarChart = ({ started }) => {
  const [progress, setProgress] = useState(0);
  const skills = [
    { label: "PHP",        value: 88 },
    { label: "Laravel",    value: 82 },
    { label: "Java",       value: 72 },
    { label: "MySQL",      value: 85 },
    { label: "REST API",   value: 80 },
    { label: "Git",        value: 78 },
  ];
  ...
  const getPoint = (i, val, scale = 1) => {
    const angle = (i / N) * 2 * Math.PI - Math.PI / 2;
    const r = (val / 100) * R * scale * progress;
  ...
  const polyPoints = skills.map((s, i) => getPoint(i, s.value)).map(p => `${p.x},${p.y}`).join(" ");
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 700, color: "var(--muted)", fontFamily: "'JetBrains Mono',monospace", letterSpacing: "2px", textTransform: "uppercase", marginBottom: 12 }}>skill radar</div>

      <div style={{ background: "rgba(6,14,30,0.6)", border: "1px solid rgba(59,130,246,0.12)", borderRadius: 16, padding: "16px 12px", display: "flex", flexDirection: "column", alignItems: "center" }}>
        <svg width={220} height={220} viewBox="0 0 220 220">
          ...
          {[0.25, 0.5, 0.75, 1].map((scale, ri) => {
            ...
            return <polygon key={ri} points={pts} fill="none" stroke="rgba(59,130,246,0.1)" strokeWidth={1} />;
          })}

          {/* Axis lines */}
          {skills.map((_, i) => {
            const angle = (i / N) * 2 * Math.PI - Math.PI / 2;
            return <line key={i} x1={cx} y1={cy} x2={cx + R * Math.cos(angle)} y2={cy + R * Math.sin(angle)} stroke="rgba(59,130,246,0.12)" strokeWidth={1} />;
          })}
            ...
            return (
              <g key={i}>
                <circle cx={pt.x} cy={pt.y} r={isHov ? 6 : 4}
                  fill={isHov ? "#06B6D4" : "#3B82F6"}
                  stroke="rgba(6,14,30,0.8)" strokeWidth={2}
                ...
                <text
                  ...
                  fill={isHov ? "#06B6D4" : "rgba(200,216,240,0.7)"}
                ...
                {/* Tooltip */}
                {isHov && (
                  <text x={pt.x} y={pt.y - 12} textAnchor="middle" fill="#06B6D4" fontSize={10} fontFamily="'Outfit',sans-serif" fontWeight={700}>{s.value}%</text>
                )}
              </g>
            );
          })}
          ...
          {skills.map((s, i) => (
            <div key={s.label}
              ...
              style={{
                ...
                fontSize: 10, color: hoveredIdx === i ? "#06B6D4" : "var(--muted)",
                fontFamily: "'JetBrains Mono',monospace", cursor: "default",
                transition: "color 0.2s",
              }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: hoveredIdx === i ? "#06B6D4" : "#3B82F6", flexShrink: 0 }} />
              {s.label} <span style={{ color: hoveredIdx === i ? "#06B6D4" : "rgba(59,130,246,0.5)" }}>{s.value}%</span>
            </div>
          ))}
        </div>
      </div>
...
const StatCard = ({ n, l, icon, delay, started }) => {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ padding: "18px 16px", background: hovered ? "rgba(29,78,216,0.12)" : "rgba(15,31,56,0.6)", border: `1px solid ${hovered ? "rgba(59,130,246,0.4)" : "rgba(59,130,246,0.12)"}`, borderRadius: 14, position: "relative", overflow: "hidden", opacity: started ? 1 : 0, transform: started ? "translateY(0)" : "translateY(20px)", transition: `opacity .55s ease ${delay}ms,transform .55s cubic-bezier(.22,1,.36,1) ${delay}ms,background .2s,border-color .2s,box-shadow .2s`, boxShadow: hovered ? "0 0 28px rgba(29,78,216,0.12)" : "none", cursor: "default" }}>
      ...
      <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 32, fontWeight: 800, color: "var(--white)", lineHeight: 1, letterSpacing: "-1.5px", display: "inline-block", animation: popped ? "statPop 0.45s cubic-bezier(.22,1,.36,1)" : "none" }}>{count}{suffix}</div>
...
const WorkChip = ({ label, delay, started }) => {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", borderRadius: 9, background: hovered ? "rgba(6,182,212,0.12)" : "rgba(6,182,212,0.05)", border: `1px solid ${hovered ? "rgba(6,182,212,0.4)" : "rgba(6,182,212,0.15)"}`, transform: hovered ? "translateY(-2px)" : started ? "translateY(0)" : "translateY(10px)", opacity: started ? 1 : 0, transition: `opacity 0.5s ease ${delay}ms,transform 0.4s cubic-bezier(.22,1,.36,1) ${delay}ms,background 0.2s,border-color 0.2s,box-shadow 0.2s`, boxShadow: hovered ? "0 4px 16px rgba(6,182,212,0.12)" : "none", cursor: "default" }}>
    ...
    <section id="about" ref={ref} style={{ background: "var(--navy-2)", borderTop: "1px solid rgba(59,130,246,0.07)" }}>
      ...
      <div className="magazine-grid" style={{ display: "grid", gridTemplateColumns: "1fr 280px 1fr", gap: "40px 36px", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ opacity: started ? 1 : 0, transform: started ? "translateY(0)" : "translateY(12px)", transition: "opacity 0.6s ease 0.5s, transform 0.6s ease 0.5s" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {["Full-time", "Hybrid", "Magang", "Kontrak"].map((t, i) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {/* ── Radar Chart (replaces terminal) ── */}
          <RadarChart started={started} />
        </div>
      </div>
      ...
      <style>{`
        ...
        @keyframes statPop{0%{transform:scale(1)}30%{transform:scale(1.18) translateY(-3px)}60%{transform:scale(0.95) translateY(1px)}100%{transform:scale(1)}}
