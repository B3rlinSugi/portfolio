    ...
    <div style={{
      ...
      <div style={{ position: "relative", width: 68, height: 68 }}>
        <svg width="68" height="68" viewBox="0 0 68 68" style={{ transform: "rotate(-90deg)" }}>
          {/* Track */}
          <circle cx="34" cy="34" r={r} fill="none" stroke="rgba(59,130,246,0.1)" strokeWidth="5" />
          {/* Progress */}
          <circle cx="34" cy="34" r={r} fill="none" stroke={skill.color} strokeWidth="5"
            strokeDasharray={circ} strokeDashoffset={dash}
            strokeLinecap="round"
            style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(.22,1,.36,1)", filter: `drop-shadow(0 0 4px ${skill.color})` }}
          />
        </svg>
        {/* Icon in center */}
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <span style={{ display: "none", fontSize: 10, color: skill.color, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace" }}>{skill.name[0]}</span>
        </div>
      </div>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--white-2)", fontFamily: "'Outfit',sans-serif" }}>{skill.name}</div>
        <div style={{ fontSize: 9, color: skill.color, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600 }}>{skill.pct}%</div>
      </div>
    </div>
  );
};
