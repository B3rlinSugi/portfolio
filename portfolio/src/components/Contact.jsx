...
const contactItems = [
  { label: "WhatsApp", value: "+62 812-9450-0613", href: "https://wa.me/6281294500613", desc: "Ping me directly",           color: "#34D399", pulse: "52,211,153",  bg: "rgba(52,211,153,0.08)",  border: "rgba(52,211,153,0.3)",
  ...
  { label: "Email",    value: "berlinsugiyanto23@gmail.com", href: "mailto:berlinsugiyanto23@gmail.com", desc: "Fastest way to reach me", color: "#FCD34D", pulse: "252,211,77", bg: "rgba(252,211,77,0.07)", border: "rgba(252,211,77,0.3)",
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="3"/><path d="m2 7 10 7 10-7"/></svg> },
  { label: "LinkedIn", value: "linkedin.com/in/berlinsugi", href: "https://linkedin.com/in/berlinsugi", desc: "Let's connect professionally", color: "#38BDF8", pulse: "56,189,248",  bg: "rgba(56,189,248,0.07)",  border: "rgba(56,189,248,0.3)",
  ...
  { label: "GitHub",   value: "github.com/B3rlinSugi", href: "https://github.com/B3rlinSugi", desc: "Browse my repositories", color: "#C8D8F0", pulse: "200,216,240", bg: "rgba(200,216,240,0.05)", border: "rgba(200,216,240,0.2)",
...
const NeonContactCard = ({ item, index, visible, prevDone, onDone }) => {
  ...
  return (
    <a href={item.href} target={item.href.startsWith("mailto") ? undefined : "_blank"} rel="noreferrer"
      ...
      style={{
        ...
        transition: `opacity 0.55s ease ${100 + index * 80}ms, transform 0.45s cubic-bezier(.22,1,.36,1) ${100 + index * 80}ms, background 0.2s`,
        /* Neon glow pulse border */
        border: `1px solid ${hovered ? item.color : "rgba(59,130,246,0.1)"}`,
        boxShadow: hovered
          ? `0 0 12px rgba(${item.pulse},0.4), 0 0 24px rgba(${item.pulse},0.2), 0 0 48px rgba(${item.pulse},0.08), inset 0 1px 0 rgba(255,255,255,0.04)`
    ...
    >
      {/* Neon corner glow */}
      <div style={{ position: "absolute", inset: 0, borderRadius: 14, pointerEvents: "none", background: hovered ? `radial-gradient(circle at 15% 50%, rgba(${item.pulse},0.08), transparent 60%)` : "transparent", transition: "background 0.25s" }} />

      <div style={{ width: 50, height: 50, borderRadius: 14, flexShrink: 0, background: hovered ? item.bg : "rgba(15,31,56,0.9)", border: `1px solid ${hovered ? item.border : "rgba(59,130,246,0.1)"}`, display: "flex", alignItems: "center", justifyContent: "center", color: item.color, transition: "all 0.25s cubic-bezier(.22,1,.36,1)", transform: hovered ? "scale(1.08) rotate(-4deg)" : "scale(1) rotate(0)", zIndex: 1, boxShadow: hovered ? `0 0 16px rgba(${item.pulse},0.4)` : "none" }}>{item.icon}</div>
  ...
  return (
    <section id="contact" ref={ref} style={{ background: "var(--navy)", borderTop: "1px solid rgba(59,130,246,0.07)", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto" }}>
        ...
        {/* Availability badge */}
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 36, opacity: visible ? 1 : 0, transition: "opacity 0.5s ease 0.2s" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)", padding: "8px 20px", borderRadius: 100 }}>
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#10B981", boxShadow: "0 0 8px rgba(16,185,129,0.6)", animation: "pulse-green 2s infinite", flexShrink: 0 }} />
            <span style={{ fontSize: 12.5, fontWeight: 600, color: "#34D399", fontFamily: "'Outfit',sans-serif" }}>Available for work</span>
            <span style={{ width: 1, height: 14, background: "rgba(16,185,129,0.25)" }} />
            <span style={{ fontSize: 11.5, color: "#6B84A8", fontFamily: "'JetBrains Mono',monospace" }}>Bekasi, West Java · Remote OK</span>
          </div>
        </div>

        {/* 2x2 Contact grid with neon glow */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }} className="contact-grid">
          {contactItems.map((item, i) => (
            <NeonContactCard key={item.label} item={item} index={i} visible={visible} prevDone={canType(i)} onDone={() => markDone(i)} />
          ))}
        </div>
      ...
      <style>{`
        ...
        @keyframes pulse-green{0%,100%{box-shadow:0 0 0 0 rgba(16,185,129,.5)}70%{box-shadow:0 0 0 8px rgba(16,185,129,0)}}
        /* Neon pulse per card */
        @keyframes neonPulse-0{0%,100%{box-shadow:0 0 6px rgba(52,211,153,0.15),0 2px 8px rgba(0,0,0,0.15)}50%{box-shadow:0 0 18px rgba(52,211,153,0.3),0 0 36px rgba(52,211,153,0.1)}}
        @keyframes neonPulse-1{0%,100%{box-shadow:0 0 6px rgba(252,211,77,0.15),0 2px 8px rgba(0,0,0,0.15)}50%{box-shadow:0 0 18px rgba(252,211,77,0.3),0 0 36px rgba(252,211,77,0.1)}}
        @keyframes neonPulse-2{0%,100%{box-shadow:0 0 6px rgba(56,189,248,0.15),0 2px 8px rgba(0,0,0,0.15)}50%{box-shadow:0 0 18px rgba(56,189,248,0.3),0 0 36px rgba(56,189,248,0.1)}}
        @keyframes neonPulse-3{0%,100%{box-shadow:0 0 6px rgba(200,216,240,0.12),0 2px 8px rgba(0,0,0,0.15)}50%{box-shadow:0 0 16px rgba(200,216,240,0.25),0 0 32px rgba(200,216,240,0.08)}}
