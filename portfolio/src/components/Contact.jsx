...
const contactItems = [
  { label: "WhatsApp", value: "+62 812-9450-0613", href: "https://wa.me/6281294500613?text=Hi%20Berlin!", desc: "Ping me directly", color: "#34D399", glow: "rgba(52,211,153,0.35)", bg: "rgba(52,211,153,0.08)", border: "rgba(52,211,153,0.25)",
  ...
  { label: "Email", value: "berlinsugiyanto23@gmail.com", href: "mailto:berlinsugiyanto23@gmail.com", desc: "Fastest way to reach me", color: "#FCD34D", glow: "rgba(252,211,77,0.35)", bg: "rgba(252,211,77,0.07)", border: "rgba(252,211,77,0.25)",
  ...
  { label: "LinkedIn", value: "linkedin.com/in/berlinsugi", href: "https://linkedin.com/in/berlinsugi", desc: "Let's connect professionally", color: "#38BDF8", glow: "rgba(56,189,248,0.35)", bg: "rgba(56,189,248,0.07)", border: "rgba(56,189,248,0.25)",
  ...
  { label: "GitHub", value: "github.com/B3rlinSugi", href: "https://github.com/B3rlinSugi", desc: "Browse my repositories", color: "#C8D8F0", glow: "rgba(200,216,240,0.25)", bg: "rgba(200,216,240,0.05)", border: "rgba(200,216,240,0.18)",
...
const ContactCard = ({ item, index, visible, prevDone, onDone }) => {
  ...
  return (
    <a href={item.href} target={item.href.startsWith("mailto") ? undefined : "_blank"} rel="noreferrer"
      ...
      style={{
        ...
        transition: `opacity 0.55s ease ${100 + index * 80}ms, transform 0.45s cubic-bezier(.22,1,.36,1) ${100 + index * 80}ms, background 0.2s, border-color 0.2s, box-shadow 0.2s`,
        boxShadow: hovered ? `0 16px 40px rgba(0,0,0,0.3), 0 0 0 1px ${item.color}18` : "none",
        position: "relative", overflow: "hidden",
      }}
    >
      <div style={{ position: "absolute", inset: 0, borderRadius: 14, pointerEvents: "none", background: hovered ? `radial-gradient(circle at 20% 50%, ${item.bg}, transparent 65%)` : "transparent", transition: "background 0.25s" }} />
      <div style={{ width: 50, height: 50, borderRadius: 14, flexShrink: 0, background: hovered ? item.bg : "rgba(15,31,56,0.9)", border: `1px solid ${hovered ? item.border : "rgba(59,130,246,0.1)"}`, display: "flex", alignItems: "center", justifyContent: "center", color: item.color, boxShadow: hovered ? `0 0 20px ${item.glow}` : "none", transition: "all 0.25s cubic-bezier(.22,1,.36,1)", transform: hovered ? "scale(1.08) rotate(-4deg)" : "scale(1) rotate(0)", zIndex: 1 }}>{item.icon}</div>
      ...
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: item.color, flexShrink: 0, zIndex: 1, opacity: hovered ? 1 : 0, transform: hovered ? "translateX(0) rotate(-45deg)" : "translateX(-8px) rotate(-45deg)", transition: "all 0.25s" }}>
    ...
    <section id="contact" ref={ref} style={{ background: "var(--navy)", borderTop: "1px solid rgba(59,130,246,0.07)", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)", padding: "8px 20px", borderRadius: 100, backdropFilter: "blur(8px)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }} className="contact-grid">
          {contactItems.map((item, i) => (
            <ContactCard key={item.label} item={item} index={i} visible={visible} prevDone={canType(i)} onDone={() => markDone(i)} />
          ))}
        </div>
