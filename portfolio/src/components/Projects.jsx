...
const ProjectCard = ({ p, index, visible }) => {
  ...
  /* Trigger draw after stagger delay */
  useEffect(() => {
    if (!visible) return;
    const t = setTimeout(() => setDrawn(true), 120 + index * 220);
  ...
  /* SVG border dimensions — updated on mount */
  const [dim, setDim] = useState({ w: 800, h: 400, r: 16 });
  useEffect(() => {
    if (!cardRef.current) return;
    const ro = new ResizeObserver(entries => {
      const { width, height } = entries[0].contentRect;
      setDim({ w: Math.round(width), h: Math.round(height), r: 16 });
    });
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 16,
        overflow: "visible",
        /* fade in the whole card slightly behind the border draw */
        opacity: drawn ? 1 : 0,
        transform: drawn ? "translateY(0)" : "translateY(18px)",
        transition: `opacity 0.5s ease ${index * 220 + 80}ms, transform 0.5s cubic-bezier(.22,1,.36,1) ${index * 220 + 80}ms`,
      }}
    >
      {/* ── SVG animated border ── */}
      <svg
      ...
      >
        <defs>
          <linearGradient id={`grad-${index}`} gradientUnits="userSpaceOnUse" x1="0" y1="0" x2={w} y2={h}>
        ...
        <rect
          ...
          rx={r} ry={r}
          fill="none"
          stroke={hovered ? `${a}30` : "rgba(59,130,246,0.1)"}
          strokeWidth={1.5}
          style={{ transition: "stroke 0.3s" }}
        />

        {/* Animated draw border */}
        <rect
          x={0.75} y={0.75}
          width={w - 1.5} height={h - 1.5}
          rx={r} ry={r}
          ...
          style={{
            transition: drawn
              ? `stroke-dashoffset 1.1s cubic-bezier(.4,0,.2,1) ${index * 220 + 140}ms`
              : "none",
            filter: `drop-shadow(0 0 6px ${a}80)`,
          }}
        />

        {/* Trailing glow dot that travels the border */}
        {drawn && (
          <circle r={3} fill={b} style={{ filter: `drop-shadow(0 0 5px ${b})` }}>
            <animateMotion
              dur="1.1s"
              begin={`${(index * 220 + 140) / 1000}s`}
              fill="freeze"
              path={`M ${r},0.75 H ${w - r} Q ${w - 0.75},0.75 ${w - 0.75},${r} V ${h - r} Q ${w - 0.75},${h - 0.75} ${w - r},${h - 0.75} H ${r} Q 0.75,${h - 0.75} 0.75,${h - r} V ${r} Q 0.75,0.75 ${r},0.75`}
            />
          </circle>
        )}
      </svg>
      ...
      <div style={{
        ...
        transition: hovered
          ? "transform 0.09s linear, box-shadow 0.2s"
        ...
        <div style={{
          ...
          background: hovered
            ? `radial-gradient(circle at ${tilt.gx}% ${tilt.gy}%, rgba(255,255,255,0.05), transparent 55%)`
            : "transparent",
        ...
        <div style={{ padding: "22px 26px", position: "relative", zIndex: 1 }} className="proj-inner">
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{
                ...
                color: a, background: `${a}10`, border: `1px solid ${a}25`,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
            {p.points.map((pt, i) => (
              <div key={i} style={{
                ...
                background: hovered ? `${a}06` : "rgba(6,14,30,0.4)",
                border: `1px solid ${hovered ? a + "15" : "rgba(59,130,246,0.07)"}`,
                transition: "all 0.2s",
              }}>
                <span style={{
                  width: 20, height: 20, borderRadius: 6, flexShrink: 0,
                  background: `${a}15`, display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 9.5, fontWeight: 700, color: a, fontFamily: "'JetBrains Mono',monospace", marginTop: 1,
                }}>{i + 1}</span>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10, paddingTop: 14, borderTop: `1px solid ${a}15` }}>
            ...
            <a href={p.github} target="_blank" rel="noreferrer" style={{
              ...
              border: `1px solid ${hovered ? a + "45" : "rgba(59,130,246,0.2)"}`,
