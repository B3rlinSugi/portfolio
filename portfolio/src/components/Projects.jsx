  ...
  return (
    <div ref={cardRef}
      ...
      style={{
        ...
        transition:`opacity .65s cubic-bezier(.22,1,.36,1) ${index*130}ms,transform .65s cubic-bezier(.22,1,.36,1) ${index*130}ms`,
      }}
    >
      <div style={{
        ...
        transform:hovered
          ?`perspective(900px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg) scale(1.02) translateZ(8px)`
          :"perspective(900px) rotateX(0) rotateY(0) scale(1)",
        transition:hovered?"transform 0.08s linear,box-shadow 0.2s,border-color 0.2s":"transform 0.55s cubic-bezier(.22,1,.36,1),box-shadow 0.4s,border-color 0.3s",
        boxShadow:hovered?`0 32px 64px rgba(0,0,0,0.4),0 0 0 1px ${a}20,inset 0 1px 0 rgba(255,255,255,0.04)`:"0 4px 20px rgba(0,0,0,0.2)",
        willChange:"transform",
      }}>
        {/* Top accent */}
        <div style={{height:3,background:`linear-gradient(to right,${a},${b})`,transition:"opacity 0.3s",opacity:hovered?1:0.5}}/>

        {/* Shine */}
        <div style={{
          position:"absolute",inset:0,borderRadius:16,pointerEvents:"none",zIndex:0,
          background:hovered?`radial-gradient(circle at ${tilt.gx}% ${tilt.gy}%,rgba(255,255,255,0.05),transparent 55%)`:"transparent",
        ...
        <div style={{padding:"22px 26px",position:"relative",zIndex:1}} className="proj-inner">
          ...
          {/* Achievements — horizontal chips style */}
            ...
            {p.points.map((pt,i)=>(
              <div key={i} style={{
                ...
                <span style={{
                  ...
                  fontSize:9.5,fontWeight:700,color:a,fontFamily:"'JetBrains Mono',monospace",
                  marginTop:1,
                }}>{i+1}</span>
