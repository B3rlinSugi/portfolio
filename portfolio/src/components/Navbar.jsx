...
/* ── Visitor Counter — pakai api.counterapi.dev (gratis, no setup) ── */
function useVisitorCount() {
  const [count, setCount] = useState(null);
  useEffect(() => {
    // Hit once per session only
    const key = "portfolio-visited";
    const hasVisited = sessionStorage.getItem(key);
    const namespace = "berlinsugi-portfolio";
    const name = "visitors";
    ...
    if (!hasVisited) {
      ...
      fetch(`https://api.counterapi.dev/v1/${namespace}/${name}`)
        .then(r => r.json())
        .then(d => setCount(d.count))
        .catch(() => {});
    }
